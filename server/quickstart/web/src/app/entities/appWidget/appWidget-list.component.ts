...
@Component({
    moduleId: module.id,
	templateUrl: 'appWidget-list.component.html',
	selector: 'appWidget-list'
})
export class AppWidgetListComponent {
    ...
    loadPage(event : LazyLoadEvent) {
        this.appWidgetService.getPage(this.example, event).
            subscribe(
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/appWidget', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let appWidgetToDelete : AppWidget = <AppWidget> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(appWidgetToDelete);
            }
        });
    }
    ...
    private delete(appWidgetToDelete : AppWidget) {
        ...
        this.appWidgetService.delete(id).
            subscribe(
                response => {
